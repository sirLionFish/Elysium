...
int move_unit(Battlefield *bf, Unit *unit, int dest_row, int dest_col) {
  ...
  // Search for the unit using name comparison instead of pointer comparison
  for (int i = 0; i < ROW_MAX; i++) {
    for (int j = 0; j < COL_MAX; j++) {
      if (bf->grid[i][j] != NULL && strcmp(bf->grid[i][j]->name, unit->name) == 0) {
        current_row = i;
        current_col = j;
        found = 1;
        break;
  ...
  if (!found) {
    printf("ERROR: Unit %s not found on the battlefield.\n", unit->name);
    return -1;
  }

  printf("Unit %s found at (%d, %d). Attempting to move to (%d, %d)\n",
          unit->name, current_row, current_col, dest_row, dest_col);
  ...
  if (move_distance > unit->stats.travel_speed) {
    printf("Unit %s can't move that far (speed: %d, attempted: %d).\n",
            unit->name, unit->stats.travel_speed, move_distance);
    return -1;
  }
